...
var move = function(obj) {
  ...
  if (dir == 0) obj.style.left = Math.min(1750, posX + acel) + "px";
  if (dir == 1) obj.style.top = Math.max(220, posY - acel) + "px";
  if (dir == 2) obj.style.left = Math.max(50, posX - acel) + "px";
  if (dir == 3) obj.style.top = Math.min(600, posY + acel) + "px";
  console.log(posX);
  console.log(posY);
...
var createCoin = function() {
  coinInterval = setInterval(function() {
    var newCoin = new Coin(
      Math.floor(Math.random() * 450 + 210) + "px",
      Math.floor(Math.random() * 1700 + 100) + "px"
    );
    //  coins.push(newCoin);
